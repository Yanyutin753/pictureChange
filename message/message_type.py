...
def on_help_reply(trigger, rules):
    help_text = "💨 结合百度AI 和 Stable-Diffusion 来文生图和图生图，suno音乐，文件和图片识别，群聊聊天模型转换\n\n"
    help_text += (
    ...
    help_text += ("\n🤖 管理员操作\n\n"
                  "认证管理员 `认证 [管理员密码]`\n\n"
                  "修改属性 `修改pictureChange参数 [参数名称] [参数数值]`\n\n"
                  "清空管理员 `清空管理员`\n\n"
                  "修改管理员密码 `修改密码 [管理员密码]`\n\n"
                  "修改群聊总功能 `群聊修改 #change [funct] to [state]`\n\n"
                  "修改已有群聊功能模式 `群聊修改 #change [funct] in [group_name] to [state]`\n\n"
                  "新增群聊模式 `群聊修改 #add [group_name]` \n\n"
                  "删除群聊模式 `群聊修改 #del [group_name]`\n\n"
                  "修改群聊模型模式 `群聊修改 #modify [group_name] to [model]`\n\n"
                  "[funct]: IMAGE / FILE / MUSIC / MODEL\n\n"
                  "[state]: true / false\n")
    return help_text
